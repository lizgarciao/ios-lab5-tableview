...
class ViewController: UIViewController, UITableViewDataSource {
    ...
    // this function gets called for each row correspoding to a movie based on the movie count defined above. If there are 20 movies, then it configures row 0, row 1, ..., row 20.
    // Disclaimer: table view only creates and configures as many cells as it needs to fit on the screen at a given time depending on how far the user has scrolled. This allows the table view to be super efficient and for cells that scroll off the screen to be reused and reconfigured as new cells that will be scrolling onto the screen.
